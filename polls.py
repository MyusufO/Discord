...
async def create_poll_message(channel, option1, option2, question=None):
    """
    Creates a Discord poll with two options and returns the sent message.
    """
    if question is None:
        question = f"Who wins? {option1} vs {option2}"

    poll = discord.Poll(question=question)
    poll.add_option(text=option1)
    poll.add_option(text=option2)
    # You can add more options if needed, but we stick with two.
